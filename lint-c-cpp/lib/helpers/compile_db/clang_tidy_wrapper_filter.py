...
"""Drop clang-tidy diagnostics in policy.unsafe_api.wrapper_files (unsafe-api pass).

Uses resolved repo-relative path equality against the manifest list — no regex.
"""
...
def _process_line(
    ...
    *,
    ...
    if path is not None:
        if path_is_unsafe_wrapper(path, repo_root):
            return True, failed, None
        if ": error:" in line or ": warning:" in line:
            failed = True
        return False, failed, line
    if skip_block:
        if _PROGRESS.match(line):
            skip_block = False
            return skip_block, failed, line
        return skip_block, failed, None
    if _PROGRESS.match(line):
        return skip_block, failed, line
...
def filter_clang_tidy_output(text: str, repo_root: Path) -> tuple[list[str], bool]:
    ...
    for line in text.splitlines():
        skip_block, failed, kept_line = _process_line(
...
def stream_clang_tidy_output(repo_root: Path, stream: io.TextIOBase) -> bool:
    """Pass progress through live; drop wrapper diagnostics. Return True if failed."""
    repo_root = repo_root.resolve()
    failed = False
    skip_block = False
    for line in stream:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        out = "\n".join(
            [
                ...
                f"{wrapper}:1:1: error: wrapper error",
                "  1 | bad();",
                "      | ^",
                "[ 2/3][0.1s] clang-tidy rel-wrapper.c",
                f"{rel_wrapper}:2:1: note: wrapper note",
                "  2 | more();",
                "      | ^",
                "[ 3/3][0.2s] clang-tidy app.c",
                f"{app}:3:1: error: app error",
                "  3 | oops();",
        ...
        with contextlib.redirect_stdout(capture):
            stream_failed = stream_clang_tidy_output(root, stream_in)
...
def main() -> int:
    ...
    if args.stream:
        return 1 if stream_clang_tidy_output(repo_root, sys.stdin) else 0
